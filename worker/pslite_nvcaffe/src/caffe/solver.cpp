...
void Solver::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
	...
	std::vector<ps::Key> keys(total_parameter);
	...
	for (int param_id = 0; param_id < this->net_->learnable_params().size();
		...
		for (int offset = 0; offset < net_params[param_id]->count(); ++ offset) {
			...
			vals[total_parameter] = param_[offset]/ps::NumWorkers();
	...
	kv_->Wait(kv_->Request(1, "", ps::kScheduler));
	// pull update from the server
	kv_->Wait(kv_->Pull(keys, weight_));
