...
#    ${bin} ${arg} &
    echo ${nodes[(( $i % ${#nodes[@]} ))]}
    ssh ${nodes[(( $i % ${#nodes[@]} ))]} "cd /localfs/xyao/caffe_pslite; examples/mnist/remote_server.sh ${DMLC_NUM_SERVER} ${DMLC_NUM_WORKER} ${bin} $i ${arg}" &
done
...
for ((i=0; i<${DMLC_NUM_WORKER}; ++i)); do
    echo ${nodes[(( $i % ${#nodes[@]} ))]}
    ssh ${nodes[(( $i % ${#nodes[@]} ))]} "cd /localfs/xyao/caffe_pslite; examples/mnist/remote_worker.sh ${DMLC_NUM_SERVER} ${DMLC_NUM_WORKER} ${worker} $i" &
