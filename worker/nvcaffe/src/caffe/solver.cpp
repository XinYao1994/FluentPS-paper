...
void Solver::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
	...
	for (int param_id = 0; param_id < this->net_->learnable_params().size();
			++param_id) {
		const float* param_ = net_params[param_id]->cpu_diff<float>();
		//keys[param_id] = param_id;
		//keys[param_id] = (param_id / num_servers) * block + param_id % num_servers;
	...
	for (int param_id = 0; param_id < this->net_->learnable_params().size(); ++param_id) {
		float* param_ = net_params[param_id]->mutable_cpu_diff<float>();
		for (int offset = 0; offset < net_params[param_id]->count(); ++offset)
		{
