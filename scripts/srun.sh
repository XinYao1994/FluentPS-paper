...
for ((i=0; i<${DMLC_NUM_WORKER}; ++i)); do
    echo ${nodes[(( $i % ${#nodes[@]} ))]}
    ssh ${nodes[(( $i % ${#nodes[@]} ))]} "cd ${project_path}/${model}; ${scripts_path}/remote_sworker.sh ${DMLC_NUM_SERVER} ${DMLC_NUM_WORKER} ${worker} $i ${solver} ${snap} ${arg}" &
done
