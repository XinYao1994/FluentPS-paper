...
[IMPORTANT] We implement SSP handler in this version (Rename to "FluentPS", which is an imcompleted version of our paper).
- `sPush(keys, values, i)`: push a list of (key, value) pairs to the server nodes in Iteration i.
- `sPull(keys, &ret, i)`: pull the values from servers for a list of keys in Itertation i.

Details and examples in "test_ssp.cc".

To use it, please acknowledge our contribution or cite this github url, many thanks
