...
class KVWorker : public SimpleApp {
 public:
  ...
  explicit KVWorker(int app_id, int customer_id) : SimpleApp() {
    ...
    slicer_ = std::bind(&KVWorker<Val>::DefaultSlicer, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
    obj_ = new Customer(app_id, customer_id, std::bind(&KVWorker<Val>::Process, this, std::placeholders::_1));
  }
  ...
  //Xin YAO: staleness push
  int sPush(const std::vector<Key>& keys,
             const std::vector<Val>& vals,
             int staleness,
             const std::vector<int>& lens = {},
             int cmd = 0,
             const Callback& cb = nullptr) {
      return ZsPush(
          SArray<Key>(keys), SArray<Val>(vals), SArray<int>(lens), staleness, cmd, cb);
    }
  ...
  //Xin YAO
  int sPull(const std::vector<Key>& keys,
             std::vector<Val>* vals,
             int staleness,
             std::vector<int>* lens = nullptr,
             int cmd = 0,
             const Callback& cb = nullptr) {
      return sPull_(SArray<Key>(keys), vals, lens, staleness, cmd, cb);
    }
  ...
  //Xin YAO: add staleness
  int ZsPush(const SArray<Key>& keys,
              const SArray<Val>& vals,
              const SArray<int>& lens = {},
              int staleness=-1,
              int cmd = 0,
      ...
      kvs.vals = vals;
      kvs.lens = lens;
      Send(ts, staleness, true, cmd, kvs);
      return ts;
    }
 ...
 private:
  ...
  template <typename C, typename D>
  int sPull_(const SArray<Key>& keys, C* vals, D* lens,
            int staleness, int cmd, const Callback& cb);
  ...
  // Xin YAO: add staleness send
  void Send(int timestamp, int staleness, bool push, int cmd, const KVPairs<Val>& kvs);
...
struct KVMeta {
  ...
  int timestamp;
  /** \brief the associated staleness */
  int staleness;
...
class KVServer : public SimpleApp {
 public:
  ...
  explicit KVServer(int app_id) : SimpleApp() {
    using namespace std::placeholders;
    obj_ = new Customer(app_id, app_id, std::bind(&KVServer<Val>::Process, this, std::placeholders::_1));
    stale = _STALE;
  }
...
void KVServer<Val>::Process(const Message& msg) {
  //std::cout << "simple_app: " << msg.meta.simple_app << ", staleness: " << msg.meta.staleness << "." << std::endl; // XinYAO
  ...
  meta.staleness = msg.meta.staleness; //Xin YAO
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
  ...
  msg.meta.staleness   = req.staleness; //Xin YAO
...
//Xin YAO: send the staleness
template <typename Val>
void KVWorker<Val>::Send(int timestamp, int staleness, bool push, int cmd, const KVPairs<Val>& kvs) {
  // slice the message
  SlicedKVs sliced;
  slicer_(kvs, Postoffice::Get()->GetServerKeyRanges(), &sliced);
  ...
  for (size_t i = 0; i < sliced.size(); ++i) {
    ...
    msg.meta.head        = cmd;
    msg.meta.timestamp   = timestamp;
    msg.meta.staleness   = staleness; //Xin YAO
	//std::cout << "In send, timestamp: " << msg.meta.timestamp << ", staleness: " << msg.meta.staleness << "." << std::endl;
    msg.meta.recver      = Postoffice::Get()->ServerRankToID(i);
    const auto& kvs = s.second;
    if (kvs.keys.size()) {
      msg.AddData(kvs.keys);
...
template <typename Val>
template <typename C, typename D>
int KVWorker<Val>::sPull_(
    const SArray<Key>& keys, C* vals, D* lens, int staleness, int cmd, const Callback& cb) {
  int ts = obj_->NewRequest(kServerGroup);
  AddCallback(ts, [this, ts, keys, vals, lens, cb]() mutable {
      mu_.lock();
      auto& kvs = recv_kvs_[ts];
  ...
  //std::cout << "In _Pull, staleness: " << staleness << "." << std::endl;
  KVPairs<Val> kvs; kvs.keys = keys;
  //Send(ts, false, cmd, kvs);
  Send(ts, staleness, false, cmd, kvs);
  return ts;
}
