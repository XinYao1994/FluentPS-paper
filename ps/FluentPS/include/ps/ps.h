...
#include <cmath> // add support for Ubuntu 18.04 + boost 1.65
...
inline Node MyNode() { return Postoffice::Get()->my_node(); }
/**
...
//Setting timeout
inline void Start(int customer_id, int timeout, const char* argv0 = nullptr) {
  Postoffice::Get()->Start(customer_id, argv0, true);
  Postoffice::Get()->van()->timeout = timeout;
}
/**
 * \brief start the system
 *
