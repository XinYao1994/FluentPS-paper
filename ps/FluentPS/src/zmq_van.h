...
class ZMQVan : public Van {
 ...
 protected:
  ...
  int Bind(const Node& node, int max_retry) override {
    ...
    const char *bindhost = Environment::Get()->find("DMLC_NODE_VHOST");
    if (bindhost){
    	hostname = node.hostname.empty() ? "*" : std::string(Environment::Get()->find("DMLC_NODE_VHOST"));
    }
    //std::string hostname = node.hostname.empty() ? "*" : node.hostname; //XinYao, using VHOST
  ...
  void Connect(const Node& node) override {
    ...
    if (GetEnv("DMLC_LOCAL", 0)) {
      std::cout << "DEBUG no DMLC_LOCAL" << std::endl;
      addr = "ipc:///tmp/" + std::to_string(node.port);
    }
    //std::cout << "node connect to scheduler" + addr << std::endl;
  ...
  int RecvMsg(Message* msg) override {
    ...
    for (int i = 0; ; ++i) {
      ...
      if (i == 0) {
        ...
        //std::cout << "After UnpackMeta: staleness: " << msg->meta.timestamp<<std::endl;
        zmq_msg_close(zmsg);
