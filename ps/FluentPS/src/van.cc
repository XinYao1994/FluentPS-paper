...
void Van::ProcessHearbeat(Message* msg) {
  ...
  for (auto &node : ctrl.node) {
    ...
    if (is_scheduler_) {
      ...
      heartbeat_ack.meta.staleness = -1; //Xin YAO
      // send back heartbeat
      Send(heartbeat_ack);
    }
  }
...
void Van::ProcessDataMsg(Message* msg) {
  ...
  auto* obj = Postoffice::Get()->GetCustomer(app_id, customer_id, this->timeout);
  CHECK(obj) << "timeout to wait App " << app_id << " customer " << customer_id \
    << " ready at " << my_node_.role; // 5 sec
  obj->Accept(*msg);
}
...
void Van::PackMeta(const Meta& meta, char** meta_buf, int* buf_size) {
  ...
  if (meta.staleness != Meta::kEmpty) pb.set_staleness(meta.staleness); //Xin YAO
...
void Van::UnpackMeta(const char* meta_buf, int buf_size, Meta* meta) {
  ...
  meta->staleness = pb.has_timestamp() ? pb.staleness() : Meta::kEmpty;
